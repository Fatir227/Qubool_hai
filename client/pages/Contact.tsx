import Layout from "@/components/Layout";
import DecorativeHeader from "@/components/DecorativeHeader";

export default function Contact() {
  return (
    <Layout>
      <section className="bg-cream section-padding">
        <div className="container mx-auto text-center">
          <DecorativeHeader
            title="Get In Touch"
            subtitle="We'd Love to Hear From You"
          />

          <div className="max-w-2xl mx-auto">
            <div className="bg-white rounded-lg border-4 border-maroon-200 p-12">
              <div className="text-6xl text-gold-500 font-script mb-6">💬</div>
              <h2 className="font-serif font-bold text-3xl text-maroon-900 mb-4">
                Coming Soon
              </h2>
              <p className="text-maroon-700 text-lg mb-6 leading-relaxed">
                Our contact form and detailed information are coming soon! In the meantime,
                you can reach us directly at:
              </p>

              <div className="space-y-4 mb-8">
                <div>
                  <p className="text-maroon-600 font-serif font-bold text-lg">
                    📞 +1 (555) 123-4567
                  </p>
                </div>
                <div>
                  <p className="text-maroon-600 font-serif font-bold text-lg">
                    📧 hello@eternalunion.com
                  </p>
                </div>
                <div>
                  <p className="text-maroon-600 font-serif font-bold text-lg">
                    📍 Bangalore, India
                  </p>
                </div>
              </div>

              <p className="text-gold-600 font-serif text-sm">
                Tell us about your vision, and we'll be in touch shortly!
              </p>
            </div>
          </div>
        </div>
      </section>
    </Layout>
  );
}
